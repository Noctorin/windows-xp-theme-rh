...
public class NumberItem extends ExtendableItem {
    ...
    public void render(RenderContext context, double mouseX, double mouseY) {
        ...
        // Windows XP style slider background with light gray gradient
        Theme.drawXPGradient(renderer, getX(), getY(), getWidth(), getHeight(false),
                NhackPlugin.theme.xpButtonTop.getValue(),
                NhackPlugin.theme.xpButtonBottom.getValue());
        
        // Slider 3D border
        Theme.drawXPBorder(renderer, getX(), getY(), getWidth(), getHeight(false),
                NhackPlugin.theme.xpHighlight.getValue(),
                NhackPlugin.theme.xpShadow.getValue());

        // Windows XP style progress bar with gradient
        if (getRenderWidth() > 0) {
            Theme.drawXPGradient(renderer, getX() + 2, getY() + 2, getRenderWidth(), getHeight(false) - 4,
        ...
        if (!subItems.isEmpty()) {
            ...
            Theme.drawXPButton(renderer, expandButtonX, expandButtonY, expandButtonSize, getHeight(),
                    NhackPlugin.theme.xpButtonTop.getValue(),
                    NhackPlugin.theme.xpButtonBottom.getValue(),
                    open);
            
            // Expand button 3D border
            Theme.drawXPBorder(renderer, expandButtonX, expandButtonY, expandButtonSize, getHeight(),
                    NhackPlugin.theme.xpHighlight.getValue(),
                    NhackPlugin.theme.xpShadow.getValue());
        }
